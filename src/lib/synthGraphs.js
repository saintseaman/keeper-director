// ─────────────────────────────────────────────────────────────
// Синтез-рецепты процедурных звуков (Web Audio).
//
// Вынесено из audioEngine.js без изменения логики: движок разросся выше
// лимита редактируемости. Каждый рецепт строит граф узлов и возвращает
// { sourceNode, gainNode, lfo?, extraNodes }, как и раньше.
//
// buildSoundGraph(ctx, masterGain, soundId, getNoise) — чистая функция:
//   ctx        — AudioContext
//   masterGain — общий выходной gain движка
//   soundId    — идентификатор рецепта
//   getNoise   — (type, duration) => AudioBuffer, кэш шума движка
// ─────────────────────────────────────────────────────────────

export function buildSoundGraph(ctx, masterGain, soundId, getNoise) {
  ...
  const noise = (type, dur) => {
    ...
    src.loop = true;
    src.buffer = getNoise(type, dur || 3);
    return src;
  };
