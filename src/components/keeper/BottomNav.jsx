...
export default function BottomNav() {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-50 bg-obsidian/95 backdrop-blur-xl border-t border-border pb-[env(safe-area-inset-bottom)]">
      <div className="max-w-lg mx-auto flex items-center justify-between px-1 py-1">
        {NAV_ITEMS.map(item => {
          ...
          return (
            <Link
              key={item.path}
              to={item.path}
              className={`flex flex-col items-center gap-0.5 py-2 px-1.5 rounded-lg transition-all flex-1 min-w-0
                ${isActive ? 'text-brass-glow' : 'text-parchment-dim'}
              `}
            >
              <Icon size={20} strokeWidth={isActive ? 2 : 1.5} />
              <span className="text-[9px] font-heading tracking-wider truncate max-w-full">{item.label}</span>
