...
Deno.serve(async (req) => {
  try {
    ...
    for (const s of sounds) {
      ...
      // Проход 2 — слушаем аудио, если по названию не хватило осей.
      const missing = allAxes.filter((a) => !(axes[a] || []).length);
      if (missing.length && s.url) {
        try {
          const prompt = [
            ...
            'Заполни ТОЛЬКО следующие оси, выбирая исключительно из допустимых значений. Если по оси ничего не подходит — пустой массив.',
            ...missing.map((a) => `- ${a}: ${AXIS_HINTS[a]} Допустимые значения: [${AXIS_VALUES[a].join(', ')}]`),
          ].join('\n');
          const schemaProps = {};
          for (const a of missing) schemaProps[a] = { type: 'array', items: { type: 'string', enum: AXIS_VALUES[a] } };
          ...
          for (const a of missing) {
            if ((cleaned[a] || []).length) axes[a] = cleaned[a];
          }
        } catch {
