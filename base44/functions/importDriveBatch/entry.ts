...
// Імпорт ПАКЕТА конкретних аудіофайлів з Google Диску за їх id.
// payload: { files: [{ id, name }] } → завантажує кожен у сховище застосунку
// та повертає [{ id, title, url, category, icon, isLoopable }].
// Працює невеликими порціями (клієнт шле по кілька файлів), щоб уникнути
// таймауту, який стається при імпорті всієї папки одним викликом.
  ...
  try {
    ...
    for (const f of files) {
      ...
      try {
        ...
        const file = new File([blob], f.name || 'drive-audio.mp3', { type: contentType });
